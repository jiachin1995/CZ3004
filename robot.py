...
class Robot:
    ...
    def __init__(self, arduino=None, fakeRun=False, fakeMap=None, stepsPerSec=1, **kwargs):
        ...
        if fakeRun:
            ...
            # fake sensors for simulation
            self.sensors = Sensors(self, fakeMap)
    ...
    def findpath(self, start=None, goal=[13, 18], waypoint=None, move=True, rowgoal=None):
        ...
        path, directions = self.pathfinder.findpath(
            start, goal, waypoint, orientation)

        if move:
            instructions = self.readDirections(directions)

            for i in instructions:
                if rowgoal and self.pos[1] == rowgoal:
                    break
                exec(i)

        if settings.logging:
            print("Movement: findpath() to " + str(goal) +
                  " with rowgoal " + str(rowgoal))
