    ...
    def getLeastSensors(self):
        ...
        for i in [1,0,2]:           #following arduino's order
            ...
            for tile in tiles_array[i]:
                if self.robot.map.getTile(tile) in [None, 1] :      #we might want to choose front over left or vice versa
                    instr += "1"
                    addzero = False
                    break
            if addzero: instr += "0"
        ...
        for row in tiles_array:
            addzero = True
            for tile in row:
                if self.robot.map.getTile(tile) in [None, 1] :      #we might want to choose left over front or vice versa
                    instr += "1"
                    addzero = False
                    break
            if addzero: instr += "0"
